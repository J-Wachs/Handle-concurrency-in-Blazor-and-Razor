...
namespace HandleConcurrencyRazorDemo.Data;
...
public class CustomerRepositoryClient(
    ...
    IUrlQueryVisitor<Entity> queryVisitor
    ) : IOptimisticConcurrencyRepository<Entity>
{
    ...
    /// <returns></returns>
    public async Task<Result<Entity?>> AddAsync(Entity entity, string userId)
    {
        string methodName = $"{nameof(AddAsync)}", paramList = "(entity, userId)";

        var url = $"{_ressourceUrl}";

        var httpResult = await httpClient.PostAsJsonAsync<Entity>(url, entity);
        var bodyString = await httpResult.Content.ReadAsStringAsync();
        if (!string.IsNullOrEmpty(bodyString))
        {
            var result = JsonSerializer.Deserialize<Result<Entity?>>(bodyString, jsonSerializerOptions);
        ...
        return Result<Entity?>.MapFromStatusCode(
    ...
    /// <returns></returns>
    public async Task<Result> DeleteAsync(Entity entity)
    {
    ...
    /// <summary>
    /// Send a Get request to REST API to retrieve a 'page' of records.
    /// </summary>
    /// <param name="filter">Filter for selection</param>
    /// <param name="pageNumber">Page number to get</param>
    /// <param name="pageSize">Number of records per page</param>
    /// <returns></returns>
    public async Task<Result<List<Entity>?>> GetAsync(Expression<Func<Entity, bool>>? filter, IEnumerable<SortDescriptor> sorting, int pageNumber, int pageSize)
    {
        ...
        {
            return Result<List<Entity>?>.CopyResult(buildUrlStringResult);
        }
        ...
        if (!string.IsNullOrEmpty(bodyString))
        {
            ...
            {
                var result = JsonSerializer.Deserialize<Result<List<Entity>?>>(bodyString, jsonSerializerOptions);
                if (result is not null)
                {
                    return result;
                }
        ...
        logger.LogCritical("An error occured in '{methodName}{parmList}'. The error is: 'Call to GetAll API returned an error. The code is {(int?)httpResult?.StatusCode} '{httpResult?.ReasonPhrase}''.",
            methodName,
            paramList,
            (int)httpResult.StatusCode, httpResult.ReasonPhrase);
        return Result<List<Entity>?>.Failure("Could not retrieve records from API.");
    }
    ...
    /// <returns></returns>
    public async Task<Result<Entity?>> GetByIdAsync(long id)
    {
    ...
    /// <returns></returns>
    public async Task<Result<Entity?>> UpdateAsync(Entity entity, string userId)
    {
    ...
    /// <returns></returns>
    private Result<string?> BuildUrlString(Expression<Func<Entity, bool>>? filter, IEnumerable<SortDescriptor> sorting, int pageNumber, int pageSize)
    {
